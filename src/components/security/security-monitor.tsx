...
export function SecurityMonitor({
  ...
  const getSecurityStatus = () => {
    if (securityScore >= 90) return { status: 'excellent', color: 'text-green-500', icon: CheckCircle };
    if (securityScore >= 70) return { status: 'good', color: 'text-yellow-500', icon: AlertTriangle };
    if (securityScore >= 50) return { status: 'fair', color: 'text-orange-500', icon: AlertTriangle };
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              <div className={cn('w-12 h-12 rounded-full flex items-center justify-center mr-4', 
                securityStatus.status === 'excellent' ? 'bg-green-500/20' :
                securityStatus.status === 'good' ? 'bg-yellow-500/20' :
                securityStatus.status === 'fair' ? 'bg-orange-500/20' : 'bg-red-500/20'
              )}>
            ...
            <div className="flex items-center space-x-2">
              <Badge 
                className={cn(
                  securityStatus.status === 'excellent' ? 'bg-green-500' :
                  securityStatus.status === 'good' ? 'bg-yellow-500' :
                  securityStatus.status === 'fair' ? 'bg-orange-500' : 'bg-red-500'
                )}
              >
                {securityStatus.status === 'excellent' ? 'Mükemmel' :
                 securityStatus.status === 'good' ? 'İyi' :
                 securityStatus.status === 'fair' ? 'Orta' : 'Kötü'}
              </Badge>
