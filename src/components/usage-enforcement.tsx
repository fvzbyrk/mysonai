        ...
        <Card className="bg-red-500/10 backdrop-blur-md border-red-500/20">
          ...
          <CardContent>
            <div className="space-y-2 text-red-300">
              {messagesOverLimit && (
                <p>• Mesaj limitiniz aşıldı ({usage.totalMessages}/{usage.monthlyLimit.messages})</p>
              )}
              {tokensOverLimit && (
                <p>• Token limitiniz aşıldı ({usage.totalTokens}/{usage.monthlyLimit.tokens})</p>
              )}
              {imagesOverLimit && (
                <p>• Görsel limitiniz aşıldı ({usage.imagesGenerated}/{usage.monthlyLimit.images})</p>
              )}
            </div>
        ...
        <Card className="bg-yellow-500/10 backdrop-blur-md border-yellow-500/20">
          ...
          <CardContent>
            <div className="space-y-2 text-yellow-300">
              {messagesNearLimit && (
                <p>• Mesaj limitinizin %80'i doldu ({usage.totalMessages}/{usage.monthlyLimit.messages})</p>
              )}
              {tokensNearLimit && (
                <p>• Token limitinizin %80'i doldu ({usage.totalTokens}/{usage.monthlyLimit.tokens})</p>
              )}
              {imagesNearLimit && (
                <p>• Görsel limitinizin %80'i doldu ({usage.imagesGenerated}/{usage.monthlyLimit.images})</p>
              )}
            </div>
            <div className="mt-4 space-y-2">
              <Button 
