...
export function EnhancedError({ 
  ...
  const getErrorIcon = () => {
    if (!isOnline) return <WifiOff className="w-8 h-8 text-red-400" />;
    if (errorInfo.type === 'network') return <Wifi className="w-8 h-8 text-yellow-400" />;
    if (errorInfo.type === 'auth') return <AlertTriangle className="w-8 h-8 text-red-400" />;
    return <Bug className="w-8 h-8 text-red-400" />;
  };

  const getErrorColor = () => {
    switch (errorInfo.severity) {
      case 'low': return 'border-yellow-500/20 bg-yellow-500/10';
      case 'medium': return 'border-orange-500/20 bg-orange-500/10';
      case 'high': return 'border-red-500/20 bg-red-500/10';
      case 'critical': return 'border-red-600/20 bg-red-600/10';
      default: return 'border-red-500/20 bg-red-500/10';
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      <div className={`max-w-md w-full backdrop-blur-md rounded-2xl p-8 border ${getErrorColor()} text-center`}>
        ...
        {showDetails && (
          <details className="text-left mb-6">
            ...
            <div className="mt-2 p-3 bg-black/20 rounded text-xs text-gray-400 font-mono">
              ...
              <p><strong>Tip:</strong> {errorInfo.type}</p>
              <p><strong>Önem:</strong> {errorInfo.severity}</p>
              {error.stack && (
