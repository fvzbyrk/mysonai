...
export function useErrorMonitoring() {
  ...
  const addError = useCallback(
    (errorInfo: ErrorInfo) => {
      ...
      // Send to analytics
      trackError(errorInfo.category, errorInfo.message, errorInfo.severity, errorInfo.stack);
  ...
  const logAPIError = useCallback(
    (endpoint: string, method: string, status: number, message: string, response?: any) => {
      const errorInfo: ErrorInfo = {
  ...
  const logReactError = useCallback(
    (error: Error, errorInfo: React.ErrorInfo, componentStack?: string) => {
