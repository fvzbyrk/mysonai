...
export function useConversion() {
  ...
  const trackGoalCompletion = useCallback(
    (goalName: string, value: number, source: string, medium: string, campaign: string) => {
  ...
  const trackCTAClick = useCallback(
    (ctaId: string, text: string, type: string, location: string) => {
      const existingCTA = ctaPerformance.find(cta => cta.id === ctaId);

      if (existingCTA) {
        setCtaPerformance(prev =>
          prev.map(cta => (cta.id === ctaId ? { ...cta, clicks: cta.clicks + 1 } : cta))
        );
      } else {
  ...
  const trackCartAbandonment = useCallback(
    (items: CartItem[], totalValue: number, reason?: string) => {
  ...
  const trackExitIntent = useCallback(
    (page: string, timeOnPage: number, scrollDepth: number, actions: string[]) => {
  ...
  const getConversionInsights = useCallback(() => {
    ...
    if (exitIntent.length > 0) {
      insights.push(
        `${exitIntent.length} çıkış niyeti tespit edildi. Exit-intent popup\'ları ekleyin.`
      );
    }
