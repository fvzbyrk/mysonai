...
export function useLogging(config: Partial<LogConfig> = {}) {
  ...
  const debug = useCallback(
    (message: string, category: string = 'general', context?: Record<string, any>) => {
  ...
  const logUserAction = useCallback(
    (action: string, target?: string, context?: Record<string, any>) => {
  ...
  const logPerformance = useCallback(
    (metric: string, value: number, unit: string = 'ms', context?: Record<string, any>) => {
  ...
  const logBusiness = useCallback(
    (event: string, value?: number, context?: Record<string, any>) => {
