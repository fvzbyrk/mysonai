// GPT + Gemini integration for existing system
...
class GPTGeminiIntegration {
  private config: AINewsConfig;
  private lastGenerated: Date | null = null;

  constructor(config: AINewsConfig) {
    this.config = config;
  }

  // Generate hybrid tech news using GPT + Gemini
  async generateHybridTechNews(): Promise<AINewsResult> {
    try {
      // Generating hybrid tech news with GPT + Gemini
  ...
  // Generate news using GPT (existing system)
  private async generateGPTNews(): Promise<AINewsItem[]> {
    try {
      // Generating news with GPT
  ...
  private async generateGeminiNews(): Promise<AINewsItem[]> {
    try {
      // Generating news with Gemini
  ...
  private combineNews(gptNews: AINewsItem[], geminiNews: AINewsItem[]): AINewsItem[] {
    const combined: AINewsItem[] = [];
  ...
  private filterByQuality(news: AINewsItem[]): AINewsItem[] {
  ...
  private calculateQualityScore(item: AINewsItem): number {
  ...
  private deduplicateAndSort(news: AINewsItem[]): AINewsItem[] {
    // Remove duplicates based on title similarity
    const unique = news.filter((item, index, arr) => {
      return (
        arr.findIndex(other => this.calculateSimilarity(item.title, other.title) > 0.8) === index
      );
    });
  ...
  private calculateQualityMetrics(
    gptNews: AINewsItem[],
    geminiNews: AINewsItem[],
    finalNews: AINewsItem[]
  ) {
    const gptQuality =
      gptNews.length > 0
        ? gptNews.reduce((sum, item) => sum + item.gptScore, 0) / gptNews.length
        : 0;

    const geminiQuality =
      geminiNews.length > 0
        ? geminiNews.reduce((sum, item) => sum + item.geminiScore, 0) / geminiNews.length
        : 0;

    const hybridQuality =
      finalNews.length > 0
        ? finalNews.reduce((sum, item) => sum + item.hybridScore, 0) / finalNews.length
        : 0;

    const averageQuality =
      finalNews.length > 0
        ? finalNews.reduce((sum, item) => sum + this.calculateQualityScore(item), 0) /
          finalNews.length
        : 0;
...
export const defaultGPTGeminiConfig: AINewsConfig = {
  useGPT: true,
  useGemini: true,
  gptWeight: 0.6,
  geminiWeight: 0.4,
