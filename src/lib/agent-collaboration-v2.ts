...
export class CollaborationStateMachine {
  ...
  private synthesizeResponse(): string {
    ...
    const responses = this.state.responses
      ...
      .slice(0, 3); // En iyi 3 yanıt
    
    return `**İşbirliği Sonucu:**\n\n${responses.map(r => 
      `**${getAgentById(r.agentId)?.name}:** ${r.response}`
    ).join('\n\n')}`;
  }
  ...
  private checkQuality(): { passed: boolean; reason?: string } {
    ...
    const avgQuality = this.state.responses.reduce((sum, r) => sum + r.quality, 0) / this.state.responses.length;
