...
export default function SupportSystemPage() {
  ...
  const mockFAQs: FAQ[] = [
    {
      ...
      answer: 'Admin paneline giriş yapın, "Blog Yönetimi" bölümüne gidin ve "Yeni Makale" butonuna tıklayın. Formu doldurup yayınlayabilirsiniz.',
      category: 'Blog',
      tags: ['blog', 'post', 'yeni'],
      isPublished: true,
      viewCount: 45,
    ...
    },
    {
      ...
      answer: 'Sistem ayarları bölümünden "API Yönetimi" sekmesine gidin ve "Yeni API Key" butonuna tıklayın. İzinleri belirleyip oluşturabilirsiniz.',
      category: 'API',
    ...
    },
    {
      ...
      answer: 'Giriş sayfasında "Şifremi Unuttum" linkine tıklayın ve e-posta adresinizi girin. Size şifre sıfırlama linki gönderilecektir.',
      category: 'Hesap',
  ...
  const mockTickets: Ticket[] = [
    {
      ...
      description: 'Oluşturduğum blog post yayınlanmıyor. Durumu "Yayınlandı" olarak görünüyor ama blog sayfasında görünmüyor.',
      status: 'in_progress',
      priority: 'high',
      category: 'bug',
      assignee: 'Admin',
    ...
    },
    {
      ...
      description: 'Gemini API entegrasyonunda hata alıyorum. API anahtarı doğru görünüyor ama çağrılar başarısız oluyor.',
      status: 'open',
      priority: 'medium',
      category: 'technical',
    ...
    },
    {
      ...
      description: 'Blog yazılarına yorum sistemi eklenebilir mi? Kullanıcıların makalelere yorum yapabilmesi güzel olur.',
      status: 'resolved',
      priority: 'low',
      ...
      messages: [
        ...
        },
        {
          ...
          author: 'Admin',
          authorEmail: 'admin@mysonai.com',
          content: 'Harika bir öneri! Yorum sistemi geliştirme planlarımızda var. Yakında eklenecek.',
          isInternal: false,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'in_progress':
        return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/50">İşlemde</Badge>;
      case 'resolved':
        return <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Çözüldü</Badge>;
      case 'closed':
  ...
  const getCategoryBadge = (category: string) => {
    switch (category) {
      ...
      case 'billing':
        return <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Faturalama</Badge>;
      case 'general':
      ...
      case 'feature_request':
        return <Badge className="bg-purple-500/20 text-purple-400 border-purple-500/50">Özellik</Badge>;
      default:
        return <Badge className="bg-gray-500/20 text-gray-400 border-gray-500/50">Bilinmiyor</Badge>;
    }
  };
  ...
  const getChatStatusBadge = (status: string) => {
    switch (status) {
      case 'waiting':
        return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/50">Bekliyor</Badge>;
      case 'active':
            ...
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{tickets.filter(t => t.status === 'open' || t.status === 'in_progress').length}</p>
                </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{liveChats.filter(c => c.status === 'active').length}</p>
                </div>
          ...
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="in_progress" className="bg-gray-800">İşlemde</option>
                      <option value="resolved" className="bg-gray-800">Çözüldü</option>
                      <option value="closed" className="bg-gray-800">Kapalı</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white/10 border border-white/20 text-white rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <option value="all" className="bg-gray-800">Tüm Öncelikler</option>
                      <option value="urgent" className="bg-gray-800">Acil</option>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="technical" className="bg-gray-800">Teknik</option>
                      <option value="billing" className="bg-gray-800">Faturalama</option>
                      <option value="general" className="bg-gray-800">Genel</option>
                      <option value="bug" className="bg-gray-800">Hata</option>
                      <option value="feature_request" className="bg-gray-800">Özellik</option>
                    </select>
                  </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              {filteredTickets.map((ticket) => (
                <Card key={ticket.id} className="bg-white/10 backdrop-blur-md border-white/20 hover:bg-white/15 transition-all duration-300">
                  <div className="p-6">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/50">{faq.category}</Badge>
                        {faq.isPublished ? (
                          <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Yayınlandı</Badge>
                        ) : (
                          <Badge className="bg-gray-500/20 text-gray-400 border-gray-500/50">Taslak</Badge>
                        )}
                      </div>
                      ...
                      {chat.messages.length > 0 && (
                        <div className="bg-white/5 p-3 rounded-lg mb-3">
                          <p className="text-gray-300 text-sm">
                            <strong>{chat.messages[chat.messages.length - 1].author}:</strong> {chat.messages[chat.messages.length - 1].content}
                          </p>
                        </div>
                      )}
                    </div>
          ...
          <Card className="bg-white/10 backdrop-blur-md border-white/20">
            <div className="p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">
                {searchTerm || selectedStatus !== 'all' || selectedPriority !== 'all' || selectedCategory !== 'all' ? 'Ticket bulunamadı' : 'Henüz ticket yok'}
              </h3>
