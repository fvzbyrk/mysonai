...
export default function AdvancedSettingsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Tamamlandı</Badge>;
      case 'failed':
        return <Badge className="bg-red-500/20 text-red-400 border-red-500/50">Başarısız</Badge>;
      case 'in_progress':
        return <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/50">Devam Ediyor</Badge>;
      default:
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      ...
      case 'incremental':
        return <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Artımlı</Badge>;
      case 'differential':
        return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/50">Diferansiyel</Badge>;
      default:
        return <Badge className="bg-gray-500/20 text-gray-400 border-gray-500/50">Bilinmiyor</Badge>;
    }
  };
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    {setting.isRequired && (
                      <Badge className="bg-red-500/20 text-red-400 border-red-500/50">Gerekli</Badge>
                    )}
                    {setting.isEncrypted && (
                      <Badge className="bg-purple-500/20 text-purple-400 border-purple-500/50">Şifreli</Badge>
                    )}
                  </div>
                </div>
        ...
        {selectedCategory === 'backup' && (
          <Card className="bg-white/10 backdrop-blur-md border-white/20">
            <div className="p-6">
              ...
              <div className="space-y-4">
                {backups.map((backup) => (
                  <div key={backup.id} className="bg-white/5 p-4 rounded-lg flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-4 mt-2">
                          <span className="text-gray-400 text-xs">{formatFileSize(backup.size)}</span>
                          <span className="text-gray-400 text-xs">{formatDate(backup.createdAt)}</span>
                ...
                {apiKeys.map((apiKey) => (
                  <div key={apiKey.id} className="bg-white/5 p-4 rounded-lg">
                    ...
                    <div className="flex items-center justify-between mt-3 text-gray-400 text-xs">
                      ...
                      {apiKey.lastUsed && (
                        <span>Son kullanım: {formatDate(apiKey.lastUsed)}</span>
                      )}
                      {apiKey.expiresAt && (
