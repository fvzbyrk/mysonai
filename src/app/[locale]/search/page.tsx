...
export default function SearchPage({ params }: { params: { locale: Locale } }) {
  ...
  const searchData: SearchResult[] = [
    // Pages
    {
      ...
      description: isTurkish ? 'MySonAI ana sayfası - AI asistanları ve çözümleri' : 'MySonAI homepage - AI assistants and solutions',
      url: '/',
      type: 'page',
      relevance: 1.0
    },
    {
      ...
      description: isTurkish ? 'Şirket hakkında bilgiler, ekip, misyon ve vizyon' : 'Company information, team, mission and vision',
      url: '/about',
      type: 'page',
      relevance: 0.9
    },
    {
      ...
      description: isTurkish ? 'AI çözümleri, klasik bilişim, yazılım ihtiyaçları' : 'AI solutions, classic IT, software needs',
      url: '/services',
      type: 'service',
      relevance: 0.9
    },
    {
      ...
      description: isTurkish ? 'MySon marka çözümleri - Video, Firmatch, Avukat, Kids, Education, Music' : 'MySon brand solutions - Video, Firmatch, Avukat, Kids, Education, Music',
      url: '/solutions',
      type: 'solution',
      relevance: 0.9
    },
    {
      ...
      description: isTurkish ? 'Plan fiyatları, özellikler ve karşılaştırma' : 'Plan prices, features and comparison',
      url: '/pricing',
      type: 'page',
      relevance: 0.8
    },
    ...
    },
    {
      ...
      description: isTurkish ? 'Sık sorulan sorular ve cevapları' : 'Frequently asked questions and answers',
      url: '/faq',
      type: 'page',
      relevance: 0.7
    },
  ...
  const getSearchSuggestions = (query: string) => {
    if (!query.trim()) return popularSearchesData.slice(0, 5);
                    ...
                    {blogCategories.map(category => (
                      <label key={category.slug} className="flex items-center text-white">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setFilters(prev => ({ ...prev, category: [...prev.category, category.slug] }));
                            } else {
                              setFilters(prev => ({ ...prev, category: prev.category.filter(c => c !== category.slug) }));
                            }
                          }}
                          className="mr-2"
                        />
