...
async function generateHybridNews(config?: Partial<HybridConfig>) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to generate hybrid news' }, { status: 500 });
  }
}
...
async function generateGPTOnlyNews() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to generate GPT-only news' }, { status: 500 });
  }
}
...
async function generateGeminiOnlyNews() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to generate Gemini-only news' }, { status: 500 });
  }
}
  ...
  try {
    ...
    const comparison = {
      ...
      hybrid: {
        ...
        avgScore:
          result.data.reduce((sum, item) => sum + item.hybridScore, 0) /
          Math.max(result.sources.hybrid, 1),
      },
    };
    ...
    return NextResponse.json({ error: 'Failed to compare sources' }, { status: 500 });
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to get hybrid status' }, { status: 500 });
  }
}
