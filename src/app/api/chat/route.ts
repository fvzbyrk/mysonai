      ...
      if (selectedAgent && lastMessage?.content) {
        ...
        if (recommendation) {
          ...
          if (selectedAgent === 'tacettin') {
            ...
            mockResponse = `Aslanım! Ben Erdem, Ege Üniversitesi Beden Eğitimi Öğretmenliği mezunu, 25+ yıl deneyimli Head Coach'un! 💪 ${lastMessage.content} konusunda sana yardım edebilirim!${fileInfo}\n\nKardeşim, bugün nasıl hissediyorsun? Hadi bakalım, ne yapacağız? Maltepe'deyken atletizm takımında çalıştığımız sporcularımız vardı, onlar da böyle başlamıştı. Profesyonel futbol, basketbol, dövüş sporları takımlarında da çalıştım, uluslararası sporcular yetiştirdim. Bilimsel temelli programlarla seni hedefine ulaştıracağım!\n\nBu demo modunda çalışıyoruz, gerçek AI yanıtları için OpenAI API key'i gerekli.`;
          } else {
